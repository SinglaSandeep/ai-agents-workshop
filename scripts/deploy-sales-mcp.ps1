<#
.SYNOPSIS
    Build and deploy the Zava SALES MCP server to Azure Container Apps.

.DESCRIPTION
    Ships src/mcp_servers/sales (FastMCP, streamable HTTP) to the ACA
    environment as the 'zava-sales-mcp' Container App with external ingress so
    the Foundry agents can reach it by URL.
...
$Service          = 'sales'
$AppName          = 'zava-sales-mcp'
$Port             = 8003
$ContainerEnvKey  = 'COSMOS_SALES_CONTAINER'
$UrlEnvKey        = 'SALES_MCP_URL'
# -----------------------------------------------------------------------------
