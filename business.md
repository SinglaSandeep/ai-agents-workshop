...
> **What this demo is.** A natural‑language **“Insights‑to‑Action”** assistant for a
> retailer. A business user (store manager, merchandiser, marketing lead) asks a
> plain‑English question; a team of AI agents pulls **insights** from three
> business domains — **Sales, Inventory, Marketing** — and an **Action agent**
> turns those insights into a short, prioritised set of **operational actions**,
> with an optional chart.
>
> **About the brand.** *Zava* is a **fictional Pacific‑Northwest DIY hardware
> retailer** used as the sample dataset. It is brand‑neutral — none of the data
> mentions any real company. The exact same pattern (sales trend + stock
> position + campaign → coordinated action) applies to any retail or CPG brand,
> e.g. a beverage company, by swapping the seed data.
...
Today those three facts live in three different systems and three different teams.
By the time someone joins them up, the moment has passed. Zava’s assistant
**joins them automatically** and recommends the move.
...
→ The assistant detects *“we’re actively promoting a product we can’t fulfil in
Seattle while the category is cooling”* and recommends an **emergency
replenishment / inter‑store transfer + a possible promo adjustment** — something
no single domain dashboard would surface on its own.
...
- **MCP servers** keep data access *outside* the model: every number is fetched
  by a typed, read‑only tool over Cosmos — the agents are told *“never invent ids
  or figures.”* That makes answers grounded and auditable.
- **Foundry IQ knowledge base** handles the *unstructured* marketing knowledge
  (campaign briefs, post‑mortems) that doesn’t fit a SQL row.
- **One terminal Action agent** means a single, well‑formatted final answer (no
  separate response‑generator hop), which also reduces token use / rate‑limit
  pressure.
