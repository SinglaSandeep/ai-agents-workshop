...
5. Register a Foundry project connection (``CustomKeys``) so the hosted
   marketing agent can call the KB over MCP using the Search admin key.
...
def _register_connection(search_endpoint: str, kb_name: str, connection_name: str) -> None:
    """Create / update the Foundry project connection used by the agent.

    Matches the pattern documented at
    https://learn.microsoft.com/azure/foundry/agents/how-to/foundry-iq-connect
    The Foundry project's system-assigned managed identity must have the
    ``Search Index Data Reader`` role on the Azure AI Search service.
    """
    mcp_target = (
        f"{search_endpoint}/knowledgebases/{kb_name}/mcp?api-version={SEARCH_API_VERSION}"
    )
