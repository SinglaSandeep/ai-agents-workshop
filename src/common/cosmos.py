"""Shared Cosmos DB client builder.

Uses key-based auth when ``COSMOS_KEY`` is set (the deployed Container Apps
inject the account key as a secret); otherwise falls back to
``DefaultAzureCredential`` for local az-login / managed-identity dev.
"""
...
def get_cosmos_client() -> CosmosClient:
    ...
    # Prefer the account key (key-based auth) when provided; fall back to
    # DefaultAzureCredential for local development without a key.
