...
```python
"""Provision the Store-Ops Foundry IQ knowledge base and the project connection."""
...
STORE_OPS_DOCS = Path(__file__).resolve().parents[1] / "knowledge_seed" / "hr"
...
def _upload_documents(endpoint: str, index_name: str) -> int:
    ...
    LOG.info("Uploaded %d store-ops docs", len(docs))
    time.sleep(3)
    return len(docs)
...
def _create_or_update_kb(endpoint: str, kb_name: str, source_index: str) -> None:
    body = {
        "name": kb_name,
        "description": "Zava store-ops policies, benefits and handbook.",
        "knowledgeSources": [{
            "name": source_index,
            "kind": "searchIndex",
        ...
        "retrievalInstructions": (
            "When answering, prefer information from Zava store-ops policies. "
...
def main() -> None:
    ...
    _register_connection(endpoint, kb_name, settings.store_ops_kb_connection_name)
