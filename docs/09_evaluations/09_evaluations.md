...
# Exercise 09 — Quality Evaluations on the Marketing Agent

## Scenario

Now that the Marketing agent is registered as a Foundry Prompt Agent
(Exercise 05), we can lean on Foundry's built-in evaluators to check **task
adherence**, **tool selection**, **groundedness**, and other quality
dimensions on a ground-truth dataset. Once we trust the bulk evaluation flow,
we'll schedule it to run on a cadence and turn on **continuous evaluation**
to sample a fraction of live agent traces for ongoing quality monitoring.
...
2. Implement and run `evaluations/quality_eval.py` against the latest agent
   version. It uploads the dataset, creates an evaluation, executes a
...
{: .success }
> - `python -m src.evaluations.quality_eval` produces a `completed` run
